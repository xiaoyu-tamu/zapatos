...
import { all, AllType, cols, Default, param, raw, SQL, sql, SQLFragment, vals } from './core';
import { completeKeysWithDefaultValue, mapWithSeparator, PromisedType } from './utils';
...
export type PromisedSQLFragmentReturnType<R extends SQLFragment<any>> = PromisedType<
  ReturnType<R['run']>
>;

// yes, the next two types are identical, but distinct names make complex inferred types more readable
export type LateralResult<L extends SQLFragmentsMap> = {
  [K in keyof L]: PromisedSQLFragmentReturnType<L[K]>;
};
export type ExtrasResult<L extends SQLFragmentsMap> = {
  [K in keyof L]: PromisedSQLFragmentReturnType<L[K]>;
};
...
type LateralOption<C extends ColumnsOption<Table>, E extends ExtrasOption> = undefined extends C
  ? undefined extends E
    ? FullLateralOption
    : LimitedLateralOption
  : LimitedLateralOption;

export interface ReturningOptionsForTable<
  T extends Table,
...
> = (undefined extends C
  ...
  : never) &
  (undefined extends E ? {} : E extends SQLFragmentsMap ? ExtrasResult<E> : never);
...
export const insert: InsertSignatures = function (
  ...
  if (Array.isArray(options.values) && options.values.length === 0) {
    ...
    const completedValues = Array.isArray(options.values)
        ? completeKeysWithDefaultValue(options.values, Default)
        : options.values,
  ...
  const completedValues = Array.isArray(options.values)
    ...
    updateValuesSQL = mapWithSeparator(updateColumns, sql`, `, (c) =>
      updateValues[c] !== undefined
        ...
        ? sql`CASE WHEN EXCLUDED.${snakeCase(c)} IS NULL THEN ${table}.${snakeCase(
            c
          )} ELSE EXCLUDED.${snakeCase(c)} END`
...
export const update: UpdateSignatures = function (
  ...
  const returningSQL = SQLForColumnsOfTable(options?.returning, table),
    extrasSQL = SQLForExtras(options?.extras),
    query = sql`UPDATE ${table} SET (${cols(options.values)}) = ROW(${vals(
      options.values
    )}) WHERE ${options.where} RETURNING ${returningSQL}${extrasSQL} AS result`;
...
> = undefined extends L
  ...
  ? PromisedSQLFragmentReturnType<L>
  : never;
...
export const select: SelectSignatures = function (
  ...
  options: SelectOptionsForTable<
    Table,
    ColumnsOption<Table>,
    LateralOption<ColumnsOption<Table>, ExtrasOption>,
    ExtrasOption
  >,
  ...
  const limit1 = mode === SelectResultMode.One || mode === SelectResultMode.ExactlyOne,
    ...
    lock =
      allOptions.lock === undefined || Array.isArray(allOptions.lock)
        ...
        : [allOptions.lock],
    order =
      allOptions.order === undefined || Array.isArray(allOptions.order)
        ? allOptions.order
        : [allOptions.order],
    ...
    distinctSQL = !distinct
      ? []
      : sql` DISTINCT${
          distinct instanceof SQLFragment || typeof distinct === 'string'
            ? sql` ON (${snakeCase(String(distinct))})`
            : Array.isArray(distinct)
            ? sql` ON (${cols(distinct)})`
            : []
        }`,
    colsSQL =
      lateral instanceof SQLFragment
        ...
        ? columns
          ? sql`count(${cols(columns)})`
          : sql<typeof alias>`count(${alias}.*)`
    ...
    colsLateralSQL =
      lateral === undefined
        ? []
        : lateral instanceof SQLFragment
        ? sql`"ljoin_passthru".result`
        : sql` || jsonb_build_object(${mapWithSeparator(
            Object.keys(lateral).sort(),
            sql`, `,
            (k, i) => sql`${param(k)}::text, "ljoin_${raw(String(i))}".result`
          )})`,
    ...
    orderSQL =
      order === undefined
        ...
        : sql` ORDER BY ${mapWithSeparator(order as OrderSpecForTable<Table>[], sql`, `, (o) => {
            ...
            return sql`${snakeCase(o.by)} ${raw(o.direction)}${
              o.nulls ? sql` NULLS ${raw(o.nulls)}` : []
            }`;
          })}`,
    offsetSQL =
      allOptions.offset === undefined ? [] : sql` OFFSET ${param(allOptions.offset)} ROWS`,
    limitSQL =
      allOptions.limit === undefined
        ? []
        : sql` FETCH FIRST ${param(allOptions.limit)} ROWS ${
            allOptions.withTies ? sql`WITH TIES` : sql`ONLY`
          }`,
    lockSQL =
      lock === undefined
        ? []
        : (lock as SelectLockingOptions[]).map((lock) => {
            // `as` clause is required when TS not strict
            const ofTables = lock.of === undefined || Array.isArray(lock.of) ? lock.of : [lock.of],
              ofClause =
                ofTables === undefined
                  ? []
                  : sql` OF ${mapWithSeparator(ofTables as Table[], sql`, `, (t) => t)}`; // `as` clause is required when TS not strict
  ...
  const rowsQuery = sql<
      SQL,
      any
    >`SELECT${distinctSQL} ${allColsSQL} AS result FROM ${table}${tableAliasSQL}${lateralSQL}${whereSQL}${groupBySQL}${havingSQL}${orderSQL}${offsetSQL}${limitSQL}${lockSQL}`,
    query =
