...
import { Column, Table, Updatable, Whereable } from 'zapatos/schema';
 ...
 */
export class ParentColumn<T extends Column = Column> {
  constructor(public value: T) {}
}
/**
 ...
 */
export function parent<T extends Column = Column>(x: T) {
    ...
    if (expression instanceof SQLFragment) {
      ...
      if (
        (expression.cast !== false &&
          (expression.cast === true || config.castArrayParamsToJson) &&
